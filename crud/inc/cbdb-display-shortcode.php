 ...
 * @since 1.0
 */
function CRUD_APP_handler($atts, $content = null) {
    // Define global DB object
    global $wpdb;

    // Define DB table name
    $cbdb_dbtable = $wpdb->prefix . 'CRUD_APP_shortcodes';
    ...
    if (!empty($cbdb_rowdata)) {
        ...
        include(CRUD_DIR . 'inc/cbdb-front-common-data.php');
        ...
        if ($get_posts->have_posts()) {
            ...
            wp_localize_script('frontend-script', 'front_ajax_object', array('ajaxurl' => admin_url('admin-ajax.php'), 'pluginurl' => CRUD_URL, 'masonryGutter' => $cbdb_grid_gap));
            ...
            <section class="<?php echo $section_class; ?>">
                <div class="cbdb-main-container <?php esc_attr_e($cbdb_main_container_class_name . $cbdb_custom_class); ?>">
                    ...
                    while ($get_posts->have_posts()) {
                        ...
                        $cbdb_template_file = CRUD_DIR . "templates/{$cbdb_layout_type}/{$cbdb_layout_type}-{$cbdb_layout_preview}.php";
                        if (file_exists($cbdb_template_file)) {
                            ...
                            echo '<p>' . esc_html__('Sorry, ' . $cbdb_layout_type . '-' . $cbdb_layout_preview . ' template file does not exist.', CRUD_TEXTDOMAIN) . '</p>';
                            exit();
                        }
                    }
                    // Reset post data
                    ...
                    <div class="cbdb-pagination-wrapper" id="cbdb-pagination-wrapper-<?php echo $cbdb_id; ?>">
                        <nav class="cbdb-pagination <?php echo $cbdb_settings['cbdb_pagination_preview']; ?>" id="cbdb_pagination" data-sc-id="<?php echo $cbdb_id; ?>">
                            ...
                            $cbdb_paginate_links = array(
                                ...
                                'prev_text' => __('&laquo; Previous', CRUD_TEXTDOMAIN),
                                'next_text' => __('Next &raquo;', CRUD_TEXTDOMAIN)
                            );
                            if (isset($cbdb_settings['cbdb_pagination_prev_text']) && !empty($cbdb_settings['cbdb_pagination_prev_text'])) {
                                $cbdb_paginate_links['prev_text'] = __($cbdb_settings['cbdb_pagination_prev_text'], CRUD_TEXTDOMAIN);
                            }
                            if (isset($cbdb_settings['cbdb_pagination_next_text']) && !empty($cbdb_settings['cbdb_pagination_next_text'])) {
                                $cbdb_paginate_links['next_text'] = __($cbdb_settings['cbdb_pagination_next_text'], CRUD_TEXTDOMAIN);
                            }
            ...
            // Else no posts found
            echo '<p>' . esc_html__('Sorry, no posts found.', CRUD_TEXTDOMAIN) . '</p>';
        }
    } else {
        // Else no layout found
        echo '<p>' . esc_html__('Sorry, no layout found.', CRUD_TEXTDOMAIN) . '</p>';
    }
}

// Register shortcode
add_shortcode('CRUD_APP', 'CRUD_APP_handler');
