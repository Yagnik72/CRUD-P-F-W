...
if (!function_exists('cbdb_posted_by')) {
    ...
    function cbdb_posted_by($author_avatar = '', $author_by = '') {
        ...
        if (!empty($author_by)) {
            $posted_by .= esc_html__($author_by . ' ', CRUD_TEXTDOMAIN);
        }
        ...
        $posted_by .= '<span class="screen-reader-text">' . esc_html__('Posted by', CRUD_TEXTDOMAIN) . '</span>
...
if (!function_exists('cbdb_get_placeholder_post_image')) {
    ...
    function cbdb_get_placeholder_post_image($image_size) {
        ...
        if (is_array($image_size) && !empty($image_size)) {
            $placeholder_post_image_url = esc_url(CRUD_URL . 'assets/images/post-placeholder.jpg');
            $width_height = 'width="' . $image_size[0] . '" height="' . $image_size[1] . '"';
        } else {
            if ($image_size == 'thumbnail') {
                $placeholder_post_image_url = esc_url(CRUD_URL . 'assets/images/post-placeholder-150x150.jpg');
                $width_height = 'width="150" height="150"';
            } elseif ($image_size == 'medium') {
                $width_height = 'width="300" height="300"';
                $placeholder_post_image_url = esc_url(CRUD_URL . 'assets/images/post-placeholder-300x300.jpg');
            } elseif ($image_size == 'medium_large') {
                $width_height = 'width="768" height="300"';
                $placeholder_post_image_url = esc_url(CRUD_URL . 'assets/images/post-placeholder-768x300.jpg');
            } elseif ($image_size == 'large') {
                $width_height = 'width="1024" height="1024"';
                $placeholder_post_image_url = esc_url(CRUD_URL . 'assets/images/post-placeholder-1024x1024.jpg');
            } else {
...
if (!function_exists('cbdb_post_date')) {
    ...
    function cbdb_post_date($post_date_link, $post_date_format = '', $date_on = '', $date_icon = false, $date_class = '') {
        ...
        if (!empty($date_on)) {
            $posted_date .= esc_html__($date_on . ' ', CRUD_TEXTDOMAIN);
        }
        ...
        if ($taxonomies && is_array($taxonomies) && count($taxonomies) > 0) {
            ...
            if ($terms) {
                ...
                foreach ($terms as $term) {
                    ...
                    $all_cats .= esc_html__($term->name, CRUD_TEXTDOMAIN);
                    // If post cat link yes
                    if ($post_cat_link == 'yes') {
                        $all_cats .= '</a>';
                    }
        ...
        if ($taxonomies && is_array($taxonomies) && count($taxonomies) > 0) {
            ...
            if ($terms) {
                ...
                foreach ($terms as $term) {
                    ...
                    $all_tags .= esc_html__($term->name, CRUD_TEXTDOMAIN);
                    // If post tag link yes
                    if ($post_tag_link == 'yes') {
                        $all_tags .= '</a>';
                    }
...
if (!function_exists('cbdb_comment_count')) {
    ...
    function cbdb_comment_count() {
        if (!post_password_required() && (comments_open() || get_comments_number())) {
            ...
            comments_popup_link(sprintf(esc_html__('Leave a comment', CRUD_TEXTDOMAIN) . '<span class="screen-reader-text">' . esc_html__('on %s', CRUD_TEXTDOMAIN) . '</span>', get_the_title()));
            echo '</span></div>';
        }
    }
    ...
    function cbdb_social_share($social_share_style) {
        ...
        $social_share = '<div class="cbdb-social-share">
            <a href="' . esc_url('https://www.facebook.com/sharer/sharer.php?u=' . get_the_permalink()) . '" target = "' . esc_attr('_blank') . '">
                <img src="' . esc_url(CRUD_URL . 'assets/images/social/facebook-' . $social_share_style . '.svg') . '" width="30" height="30" alt="' . esc_attr('Facebook-' . $social_share_style, CRUD_TEXTDOMAIN) . '">
            </a>
            <a href="' . esc_url('https://twitter.com/intent/tweet?url=' . get_the_permalink()) . '" target="' . esc_attr('_blank') . '">
                <img src="' . esc_url(CRUD_URL . 'assets/images/social/twitter-' . $social_share_style . '.svg') . '" width="30" height="30" alt="' . esc_attr('Twitter-' . $social_share_style, CRUD_TEXTDOMAIN) . '">
            </a>
            <a href="' . esc_url('https://www.linkedin.com/shareArticle?mini=true&url=' . get_the_permalink()) . '" target="' . esc_attr('_blank') . '">
                <img src="' . esc_url(CRUD_URL . 'assets/images/social/linkedin-' . $social_share_style . '.svg') . '" width="30" height="30" alt="' . esc_attr('LinkedIn-' . $social_share_style, CRUD_TEXTDOMAIN) . '">
            </a>
        </div>';
        return $social_share;
    }
